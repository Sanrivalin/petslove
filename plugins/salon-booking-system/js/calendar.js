...
(function($) {
	...
	Calendar.prototype._addServiceToAssistance = function(
		...
		var objs = this.applyAdvancedFilters([
			{
				id: e.id,
				title: is_main ? e.title : item.title,
				from: is_main ? e.from : "",
				to: is_main ? e.to : "",
		...
		if ($self.options._assistants_mode) {
			...
			$.each(data.events, function(k, e) {
				...
				if (e._end > end.getTime()) {
					...
					time_end = e.end_hour.split(' ')[2].split(':');
				...
				var parallelServiceProcessed = false;
                                ...
                                var items = e.items.filter((i) => {
                                    var duration = i.duration.split(":");
	...
	Calendar.prototype._week = function(event) {
		...
		if(this.options._assistants_mode){
			...
			t.assistants_order = sln_assistants_order;
	...
	Calendar.prototype._update_day_prepare_sln_booking_editor = function() {
		...
		$(".booking_tool_item .sln-icon--trash")
			.off("click")
		...
		$(".booking_tool_item .sln-icon--copy").off('click')
			.on('click', function(event){
				event.preventDefault();
				bookingCopy = 'copy';
				bookingId = $(this)
