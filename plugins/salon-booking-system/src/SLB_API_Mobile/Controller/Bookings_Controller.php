...
class Bookings_Controller extends REST_Controller
{
    ...
    public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/upcoming', array(
            array(
                ...
                'permission_callback' => array( $this, 'get_items_permissions_check' ),
                'args' => apply_filters('sln_api_bookings_register_routes_get_upcoming_items_args', array(
                    'hours' => array(
    ...
    public function get_items( $request )
    {
        ...
        $prepared_args['post_status'] = $request->get_param('statuses');
	...
	$prepared_args = apply_filters('sln_api_bookings_get_items_prepared_args', $prepared_args, $request);
        ...
        try {
            foreach ( $query->posts as $booking ) {
                ...
                if (!$isCurrUserAtt || empty($assistantsIDs) || ($isCurrUserAtt && array_intersect($assistantsIDs, $data->getAttendantsIds()))) {
	...
	if ($from_date === $to_date) {
	    ...
	    $query1 = new WP_Query( apply_filters('sln_api_bookings_get_upcoming_items_prepared_args', $prepared_args1, $request) );
	    $posts  = $query1->posts;
	    ...
	    $query1 = new WP_Query( apply_filters('sln_api_bookings_get_upcoming_items_prepared_args', $prepared_args1, $request) );
	    $query2 = new WP_Query( apply_filters('sln_api_bookings_get_upcoming_items_prepared_args', $prepared_args2, $request) );
	    $query3 = new WP_Query( apply_filters('sln_api_bookings_get_upcoming_items_prepared_args', $prepared_args3, $request) );
    ...
    public function prepare_response_for_collection($booking)
    {
        ...
        $tmp_discounts  = $booking->getMeta('discounts') ? $booking->getMeta('discounts') : array();
        $discounts      = array();

        foreach($tmp_discounts as $discountID) {
            $discount = new \SLB_Discount_Wrapper_Discount($discountID);
            $discounts[] = array(
                'id'            => $discount->getId(),
                'name'          => $discount->getName(),
                'amount'        => $discount->getAmount(),
                'type'          => $discount->getAmountType(),
                'amount_string' => $discount->getAmountString(),
            );
        }
        ...
        $photos = $booking->getCustomer() ? $booking->getCustomer()->getPhotos() : array();
    ...
    public function create_item( $request )
    {
	    ...
	    if ( ! $customer_id && $request->get_param('customer_email') && $request->get_param('save_as_new_customer') ) {
	...
	$bb = new SLN_Wrapper_Booking_Builder(SLN_Plugin::getInstance());
        ...
        foreach($booking_fields as $field) {
            ...
            $value = $custom_fields[$field['key']] ? $custom_fields[$field['key']] : '';
            if ($field->isCustomer() && $customer_id) {
                update_user_meta($customer_id, '_sln_' . $field['key'], $value);
            } else {
                $bb->set($field['key'], $value);
            }
        }
	...
	do_action('sln_api_bookings_update_item_post_before_valid', $bb, $request);
        ...
        foreach($booking_fields as $field) {
            ...
            if ($field->isCustomer() && $customer_id) {
                ...
                $meta['_sln_booking_' . $field['key']] = $value;
            }
        }
    ...
    public function get_item_schema()
    {
        $schema = array(
            ...
            'properties' => array(
                ...
                'customer_photos' => array(
                    'description' => __( 'Photos of the customer.', 'salon-booking-system' ),
                    'type'        => 'array',
                    'items'       => array(
                        'type' => 'object',
                        'properties' => array(
                ...
                'custom_fields' => array(
                    'description' => __( 'The custom fields for the resource.', 'salon-booking-system' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'  => array(
                        'description' => __( 'The custom field item.', 'salon-booking-system' ),
                        'type'        => 'object',
                        'context'     => array( 'view', 'edit' ),
                        'required'    => array( 'key', 'value' ),
                        'properties'  => array(
                            'key' => array(
                                'description' => __( 'The key', 'salon-booking-system' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'arg_options' => array(
                                    'required' => true,
                            ...
                            'type' => array(
                                'description' => __( 'The type', 'salon-booking-system' ),
                                'type'        => 'string',
                                'context'     => array( 'view' ),
                            ),
                        ),
