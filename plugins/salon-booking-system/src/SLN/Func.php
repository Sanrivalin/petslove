...
class SLN_Func
{
    ...
    {
        $datediff = abs((floor($from->format('U') / (60 * 60 * 24))) - floor($to->format('U') / (60 * 60 * 24)));
    ...
    static function currPageUrl($useHomeUrl = true)
    {
	if ($useHomeUrl) {
	    $homeUrl  = get_home_url();
	    $pageURL  = sanitize_text_field(implode('/', array_slice(explode('/', $homeUrl), 0, 3)));
    ...
    public static function getIntervalItemsShort()
    {
        return array(
            ...
            '+23 hours' => '23 ' . __('hours', 'salon-booking-system'),
    ...
    public static function getMinutesIntervals($interval = null, $maxItems = null)
    {
        ...
        do {
            ...
            $curr = $curr->add(new DateInterval('PT'.((int)$interval).'M'));
            $maxItems--;
    ...
    public static function updateOption($key, $value, $autoload = null) //need for updating options on all sites of multisite
    {
        if(!is_multisite()) {
            ...
            foreach ($sites as $site) {
                update_blog_option($site->id, $key, $value);
            }
        }
    }

    public static function deleteOption($key) //need for deleting options on all sites of multisite
    {
        if(!is_multisite()) {
            delete_option($key);
        } else {
