...
class SLN_Metabox_Helper
{
    ...
    public static function isMetaNewForPost($post_id, $meta) { //except _sln_booking_services (it changes after evalBookingServices())
        $is_new = false;
        foreach ($meta as $meta_key => $new_meta_value) {
            $meta_value = get_post_meta($post_id, $meta_key, true);
            if(in_array($meta_key, array('_sln_booking_services', '_sln_booking_dont_notify_customer'))) {
                continue;
            }
            if (($new_meta_value && $new_meta_value != $meta_value) ||
